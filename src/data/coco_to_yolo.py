...
def write_dataset_yaml(dst_root, task):
    ...
    The path is absolute: these files are regenerated per machine under a
    gitignored folder, so absolute paths cost nothing and remove any dependence
    on which directory the training command is run from.
    """
    base = dst_root / f"cardd_{task}"
    lines = [
